...
class TCP():
    ...
    """
    def __init__(self, port:int=200, exchange:str="STU", keep_alive:bool=True) -> None:
        self.name = uuid4()
                    ...
                    try:
                        ...
                        try:
                            if zeit > self.live_data_raw[isin][art]["TIME"]:
                                ...
                                self.data.append({"isin": isin, "type": art, "preis": preis, "zeit": zeit})
                        except TypeError:
                            pass
                    except KeyError:
                        pass
            ...
            try:
                try:
                    if float(data[summe].split("=")[1]):
                        ...
                        if "TIME" in key:
                            value = datetime.fromtimestamp(int(data[summe].split("=")[1].strip())/1000.0)
                except ValueError:
                    if data[summe].split("=")[1].isdigit():
                        ...
                        if key == 'EXCHANGES':
                            datas = [e for e in data[summe].split("=")[1].strip().split(",")]
                            value = datas
                result[key] = value
            except:
                pass
    ...
    def sub(self, isin:str, mode:str="ALL") -> dict:
        ...
        self.send(search)
        self.live_data_raw[isin.upper()] = {"ASK": {"TIME": datetime.now().replace(hour=8, minute=0, second=0, microsecond=0)}, "BID":{"TIME": datetime.now().replace(hour=8, minute=0, second=0, microsecond=0)}}
        sleep(0.5)
